...
async function httpCheck(host) {
  try {
    ...
    clearTimeout(id);
    return resp.ok || (resp.status >= 300 && resp.status < 400) || resp.status === 401;
  } catch (err) {
    return false;
  }
}
...
async function checkHost(host, ports) {
  ...
  const healthy = pingOk && (httpOk || Object.values(portResults).some(Boolean));
...
export async function main() {
  ...
  if (argHost) {
    routers.push({ name: 'arg', host: argHost, ports: parsePorts(process.env.ROUTER_PORTS) });
  } else {
    ...
    if (bHost) {
      routers.push({ name: 'beryl', host: bHost, ports: parsePorts(process.env.BERYL_PORTS || process.env.ROUTER_BERYL_PORTS) });
    }
    if (tHost) {
      routers.push({ name: 'telenet', host: tHost, ports: parsePorts(process.env.TELENET_PORTS || process.env.ROUTER_TELENET_PORTS) });
    }

    // If none found, fallback to generic ROUTER_HOST (single)
    if (!bHost && !tHost) {
      const host = process.env.ROUTER_HOST || DEFAULT_HOST;
      routers.push({ name: 'router', host, ports: parsePorts(process.env.ROUTER_PORTS) });
    }
  }

  console.log(JSON.stringify({ note: 'Starting router checks', routers: routers.map(r => ({ name: r.name, host: r.host, ports: r.ports })) }, null, 2));
...
const isMain = path.basename(process.argv[1] || '') === path.basename(new URL(import.meta.url).pathname);
if (isMain) {
  main().catch(err => {
