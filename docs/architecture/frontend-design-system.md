...
description: Dark-luxury bento design system with OKLCH tokens, Geist typography, motion foundations, and the ADR-028 liquid-glass material layer with static atmosphere backdrop.
aliases:
  [
    design system,
    dark luxury,
...
> [!abstract] Overview
> Watchman's frontend design system is a dark-luxury layer built on OKLCH color space, Geist Variable fonts, and a primitive component layer. **Phase 2** (2026-04-18) established the token foundation, typography, motion, and 14 primitives. **ADR-028** (2026-06-13) layered a liquid-glass material system and a static atmosphere backdrop on top, applying it app-wide across tiles, nav, dialogs, sheets, and popovers. The system prioritizes depth, hierarchy, and motion while preserving the cold-gold bento identity.

> [!info] Phase history
> Phase 2 (design system + primitives) shipped 2026-04-18. ADR-028 (liquid-glass + observability cards) shipped 2026-06-13. See [[docs/adr/014-time-series-duckdb-and-bento-design-system|ADR-014]] and [[docs/adr/028-liquid-glass-observability-tiles|ADR-028]] for architectural context.
...
```css
.animate-tile-enter-0 {
  animation: tile-enter 400ms var(--ease-out-q) 0ms;
}
.animate-tile-enter-1 {
  animation: tile-enter 400ms var(--ease-out-q) 20ms;
}
/* ... up to 15 */
```
...
## Glass Material System (ADR-028)

Introduced in [[docs/adr/028-liquid-glass-observability-tiles|ADR-028]]. Source files: `[[apps/frontend/src/styles/glass.css]]` (utilities) and new atmosphere/grid tokens in `[[apps/frontend/src/styles/tokens.css]]`.

### Glass Utilities (`glass.css`)

| Class            | Blur   | Saturation | Usage                                                                        |
| ---------------- | ------ | ---------- | ---------------------------------------------------------------------------- |
| `.glass-thin`    | `8px`  | `140%`     | `TabsList` bar                                                               |
| `.glass-regular` | `16px` | `160%`     | `ServiceTile` container, hero card, settings list cards, `ChartsPanel` cards |
| `.glass-thick`   | `24px` | `180%`     | `Dialog`, `Sheet`, `Popover` — highest overlay surfaces                      |
| `.glass-topbar`  | `20px` | `170%`     | `TopNav` — fixed header bar                                                  |

Each class applies:

- A translucent multi-stop OKLCH gradient background (tuned ~20–24% L, lighter than the ~14% L canvas so panels read as surfaces)
- An inset specular `::before` pseudo-element (top edge highlight)
- `backdrop-filter: blur(Xpx) saturate(Y%)` — the `saturate()` is what makes the backdrop glow through rather than read as fog

### Tone Modifiers

`.glass-tone-ok`, `.glass-tone-warn`, `.glass-tone-crit` — apply a status-tinted 1px top hairline. Used on `ServiceTile` to reflect current service health.

`.glass-interactive` — adds hover (`brightness(1.06)`) and active (`brightness(0.97)`) transitions for clickable glass surfaces.

### Atmosphere Tokens (`tokens.css`)

| Token         | Purpose                                         |
| ------------- | ----------------------------------------------- |
| `--atmo-gold` | Gold radial wash color for the atmosphere layer |
| `--atmo-cool` | Faint cool counter-wash for depth               |
| `--grid-line` | Blueprint grid line color/opacity               |

### Static Atmosphere (`.atmosphere`)

The `.atmosphere` class is applied to a fixed full-screen `<div>` rendered **once at the App root** (`App.tsx`), behind every route via `z-index: -1`:

```
.atmosphere
  ├── gold radial wash (top-left, warm)
  ├── cool counter-wash (bottom-right, subtle)
  ├── blueprint grid (CSS repeating-linear-gradient)
  └── film grain (SVG noise filter, low opacity)
```

This layer is what glass panels refract — without it `backdrop-filter` has nothing to catch. Because it is rendered once at the root, all routes (dashboard, settings, 404) share the same backdrop at zero cost. Page containers are `background: transparent`; `body` paints `--surface-0`.

### Accessibility Fallbacks

All glass classes include media/feature query fallbacks:

| Condition                         | Fallback                                              |
| --------------------------------- | ----------------------------------------------------- |
| `prefers-reduced-transparency`    | Near-opaque surface (`--surface-2`), no blur, no wash |
| `prefers-contrast`                | Raised opacity + stronger `--hairline-strong` border  |
| `prefers-reduced-motion`          | No transition on `.glass-interactive`                 |
| `@supports not (backdrop-filter)` | Opaque solid surface fallback                         |

### Application Map

| Component                          | Glass class                     |
| ---------------------------------- | ------------------------------- |
| `TopNav`                           | `glass-topbar`                  |
| `ServiceTile`                      | `glass-regular` + tone modifier |
| `Dialog` / `Sheet`                 | `glass-thick`                   |
| `Popover` (incl. profile switcher) | `glass-thick`                   |
| `TabsList`                         | `glass-thin`                    |
| Settings list/section cards        | `glass-regular`                 |
| Detail-sheet hero card             | `glass-regular`                 |
| `ChartsPanel` cards                | `glass-regular`                 |

### Service Visuals (`serviceVisuals.ts`)

`[[apps/frontend/src/lib/serviceVisuals.ts]]` is the companion module to the glass tile redesign. It provides:

- **Per-service icon map** — maps each `ServiceKind` to an SVG/emoji glyph used as the tile watermark and the detail-sheet hero watermark.
- **`heroState(value, label)`** — converts a boolean metric value into a state chip string (`"✓ Reachable"` / `"✕ Unreachable"`). Shared by `ServiceTile` and `ServiceDetailSheet`.

When adding a new service integration, add an entry to this map so the tile has identity immediately.
...
- [[apps/frontend/src/styles/tokens.css|tokens.css]] — CSS token definitions (includes `--atmo-*` and `--grid-line` atmosphere tokens)
- [[apps/frontend/src/styles/glass.css|glass.css]] — Glass material utilities (`.glass-thin/regular/thick/topbar`, tone modifiers, atmosphere)
...
- [[apps/frontend/src/lib/serviceVisuals.ts|serviceVisuals.ts]] — Per-service icon map + `heroState()` bool-hero helper
...
- [[docs/adr/014-time-series-duckdb-and-bento-design-system|ADR-014]] — Bento design system decision
- [[docs/adr/028-liquid-glass-observability-tiles|ADR-028]] — Liquid-glass material + observability-card tiles
