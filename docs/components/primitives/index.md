...
date: 2026-06-13
tags:
  [primitive, component, index, design-system, dark-luxury, glass, liquid-glass]
description: Index of all primitive components in the dark-luxury design system, including the ADR-028 glass material variants applied to Surface, Dialog, Sheet, Popover, and Tabs.
...
> [!info] Phase 2 + ADR-028
> Phase 2 (design system + primitives) shipped on 2026-04-18. [[docs/adr/028-liquid-glass-observability-tiles|ADR-028]] (2026-06-13) layered the liquid-glass material system on top: `Surface` gained `material="glass"`; `Dialog` and `Sheet` use `glass-thick`; `Popover` uses `glass-thick`; `TabsList` uses `glass-thin` with `accent-soft` active highlight. `Sparkline` was fully rewritten with smooth midpoint curves, vertical headroom, and `stretch` mode.
...
- [[docs/architecture/frontend-design-system|Frontend Design System]] — Tokens, typography, motion, elevation, and glass material system
- [[apps/frontend/src/styles/tokens.css|tokens.css]] — CSS token definitions (includes `--atmo-*`, `--grid-line`)
- [[apps/frontend/src/styles/glass.css|glass.css]] — Glass utility classes (`.glass-thin/regular/thick/topbar`)
...
- [[apps/frontend/src/lib/serviceVisuals.ts|serviceVisuals.ts]] — Per-service icon map + `heroState()` helper
...
- [[docs/adr/028-liquid-glass-observability-tiles|ADR-028]] — Glass material + observability-card tiles
