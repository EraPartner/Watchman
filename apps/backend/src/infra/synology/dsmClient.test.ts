...
function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  req.on("end", () => {
    ...
    state.calls.push({ path: url.pathname, rawUrl: req.url ?? "/", params });
...
describe("createDsmClient", () => {
  ...
  it("retries with fresh SID on error code 105", async () => {
    ...
    expect(data.model).toBe("DS920+");
    expect(state.loginCallCount).toBe(1);
    const entryCalls = state.calls.filter(
      (c) => c.path === "/webapi/entry.cgi"
    );
  ...
  it("never puts credentials or session ids in the request URL", async () => {
    const client = createDsmClient({
      http: createHttpClient(),
      config: makeConfig(),
    });
    ...
    for (const call of state.calls) {
      expect(call.rawUrl).not.toMatch(/passwd=/);
      expect(call.rawUrl).not.toMatch(/_sid=/);
      expect(call.rawUrl).not.toMatch(/secret/);
    }
  });
  ...
  it("persists acquired SID across subsequent calls", async () => {
    ...
    expect(entryCalls.every((c) => c.params["_sid"] === "persistent-sid")).toBe(
      true
    );
  });
});
