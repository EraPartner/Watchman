...
import type { SnmpGetter, SnmpWalkRequest, SnmpWalkResult } from '../../../infra/snmp/snmpGetter.js';
...
function fakeSnmp(walkMap: Record<string, Array<{ oid: string; value: string }>>): SnmpGetter {
  return {
    get: async () => ({ values: [] }),
...
function failSnmp(): SnmpGetter {
  return {
    ...
    walk: async () => { throw new Error('snmp unreachable'); },
  };
}
...
describe('RouterService SNMP stats (R1)', () => {
  it('getStats includes sysUptime when snmpCommunity set', async () => {
    const snmp = fakeSnmp({
      [SNMP_OID.sysUpTime]: [{ oid: `${SNMP_OID.sysUpTime}.0`, value: '50000' }],
    });
    const svc = new RouterService({ ping: fakePing({ success: true }), tcp: fakeTcp({}), snmp, config: snmpConfig(), now: () => 0 });
    const res = await svc.getStats(new AbortController().signal);
    expect(res.ok).toBe(true);
  ...
  it('connectedClients = count of ARP table rows', async () => {
    const snmp = fakeSnmp({
      [SNMP_OID.arpTable]: [
        { oid: `${SNMP_OID.arpTable}.1.192.168.1.10`, value: '00:11:22:33:44:55' },
        { oid: `${SNMP_OID.arpTable}.1.192.168.1.20`, value: 'aa:bb:cc:dd:ee:ff' },
        { oid: `${SNMP_OID.arpTable}.1.192.168.1.30`, value: '11:22:33:44:55:66' },
      ],
    });
    const svc = new RouterService({ ping: fakePing({ success: true }), tcp: fakeTcp({}), snmp, config: snmpConfig(), now: () => 0 });
    const res = await svc.getStats(new AbortController().signal);
  ...
  it('ifInOctets + ifOutOctets summed across interfaces', async () => {
    const snmp = fakeSnmp({
      ...
      [SNMP_OID.ifInOctets]: [{ oid: `${SNMP_OID.ifInOctets}.1`, value: '1000' }],
      [SNMP_OID.ifOutOctets]:[{ oid: `${SNMP_OID.ifOutOctets}.1`,value: '2000' }],
    });
    const svc = new RouterService({ ping: fakePing({ success: true }), tcp: fakeTcp({}), snmp, config: snmpConfig(), now: () => 0 });
    const res = await svc.getStats(new AbortController().signal);
    expect(res.ok).toBe(true);
