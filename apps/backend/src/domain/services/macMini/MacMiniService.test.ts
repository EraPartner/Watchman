...
type ExecMap = Record<
  string,
  SshExecResult | ((req: SshExecRequest) => SshExecResult)
>;

// Stats run as a single compound command; the fake synthesizes the compound
// output from a per-command fixture map (positional, delimiter-joined).
function fakeSsh(map: ExecMap, calls: SshExecRequest[] = []): SshExecutor {
  return {
    exec: async (req) => {
      calls.push(req);
      const segments = MAC_STATS_COMMANDS.map((command) => {
        const entry = map[command];
        if (!entry) return "";
        const res = typeof entry === "function" ? entry(req) : entry;
...
describe("MacMiniService", () => {
  ...
  it("getStats parses load, disk, uptime, temp", async () => {
    ...
    // one compound SSH exec carrying every stats command
    expect(calls).toHaveLength(1);
    for (const command of MAC_STATS_COMMANDS) {
      expect(calls[0]?.command).toContain(command);
    }
  ...
  it("getStats tolerates a failing core command while the other works", async () => {
    const svc = new MacMiniService({
      ping: fakePing({ success: true }),
      ssh: fakeSsh({
    ...
    expect(res.ok).toBe(true);
    if (res.ok) {
      expect(res.value.metrics.cpuLoad).toBe(0);
      expect(res.value.metrics.diskUsagePercent).toBe(3);
    }
  });

  it("getStats maps empty compound output to UnavailableError", async () => {
    const svc = new MacMiniService({
      ping: fakePing({ success: true }),
      ssh: fakeSsh({}),
      config: makeConfig(),
  ...
  describe("extended metrics", () => {
    it("parses memory from vm_stat", async () => {
      ...
      expect(m["memTotalBytes"]).toBe(
        (20000 + 15000 + 12000 + 3000 + 10000 + 5000) * 16384
      );
    });
    ...
    it("all extended commands run in parallel with core commands", async () => {
      ...
      // every command travels in the single compound exec
      expect(calls).toHaveLength(1);
      const compound = calls[0]?.command ?? "";
      for (const command of [
        ...
        expect(compound).toContain(command);
      }
    });
  });
});
