...
import type { ProfileRouteDeps } from "./profiles.js";
...
function makeStoredService(
  overrides: Partial<StoredService> = {}
): StoredService {
  return {
    id: "svc-uuid-1",
    kind: "bitcoin",
...
function makeRedacted(
  overrides: Partial<RedactedService> = {}
): RedactedService {
  return {
    id: "svc-uuid-1",
    kind: "bitcoin",
...
function makeFakeProfileDeps(store: ConfigStore, lifecycle: ServiceLifecycle) {
  const profiles = {
    ...
    serviceCounts: vi.fn(async () => ({})),
    getActiveProfileId: vi.fn(async () => "profile-default"),
  ...
  return {
    profiles: profiles as unknown as ProfileRouteDeps["profiles"],
    store,
    lifecycle,
    detector: detector as unknown as ProfileRouteDeps["detector"],
  };
}

function makeFakeLifecycle(): ServiceLifecycle {
  return {
    start: vi.fn(),
    stop: vi.fn(),
    reloadAll: vi.fn(),
...
function makeApp(store: ConfigStore, registry = new ServiceRegistry()) {
  ...
  return buildServer({
    ...
    profiles: makeFakeProfileDeps(store, fakeLifecycle),
...
describe("GET /config/kinds", () => {
  it("returns all service kinds with metadata", async () => {
    ...
    const body = res.json() as {
      data: Array<{ kind: string; label: string; description: string }>;
    };
...
describe("GET /config/load-errors", () => {
  it("returns rows that failed to load", async () => {
    const store = makeFakeStore({
      loadErrors: vi.fn(async () => [
        {
          id: "svc-uuid-9",
    ...
    const app = await makeApp(store);
    const res = await app.inject({ method: "GET", url: "/config/load-errors" });
    await app.close();

    expect(res.statusCode).toBe(200);
    const body = res.json() as {
      data: Array<{ id: string; kind: string; message: string }>;
    };
    expect(body.data).toHaveLength(1);
    expect(body.data[0]?.id).toBe("svc-uuid-9");
    expect(body.data[0]?.message).toBe("decrypt failed");
  });
});
...
describe("GET /config/services/:id", () => {
  ...
  it("returns redacted service when found", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect((res.json() as { data: RedactedService }).data.id).toBe(
      "svc-uuid-1"
    );
  });
});
...
describe("POST /config/services", () => {
  ...
  it("returns 400 on validation error", async () => {
    const store = makeFakeStore({
      create: vi.fn(async () => {
        throw new Error("invalid input: missing host");
      }),
    });
    const app = await makeApp(store);
    const res = await app.inject({
...
describe("PUT /config/services/:id", () => {
  it("returns updated service on success", async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect((res.json() as { data: RedactedService }).data.instanceId).toBe(
      "updated"
    );
  });
  ...
  it("returns 404 when service not found", async () => {
    const store = makeFakeStore({
      update: vi.fn(async () => {
        throw new Error("Not found: missing-id");
      }),
    });
    const app = await makeApp(store);
    const res = await app.inject({
...
describe("POST /config/services/:id/test", () => {
  ...
  it("returns 404 when stored service is not found", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: "/config/services/no-id/test",
      payload: {},
    });
    await app.close();
  ...
  it("returns 409 when service is not live in registry", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: "/config/services/svc-uuid-1/test",
      payload: {},
    });
    await app.close();
