...
export function configRoutes(deps: ConfigRouteDeps): FastifyPluginAsync {
  ...
  return async (app) => {
    ...
    app.get<{ Params: { id: string } }>('/config/services/:id', async (req, reply) => {
      const s = await store.get(req.params.id);
      if (!s) return reply.status(404).send({ error: { code: 'NOT_FOUND', message: 'not found' } });
    ...
    app.post<{ Body: unknown }>('/config/services', async (req, reply) => {
      try {
        ...
        return reply.status(400).send({
          error: { code: 'VALIDATION', message: e instanceof Error ? e.message : 'invalid input' },
        });
      }
    });
    ...
    app.put<{ Params: { id: string }; Body: unknown }>(
      ...
      async (req, reply) => {
        try {
          const actor = (req.headers['x-actor'] as string) ?? null;
          const updated = await store.update(req.params.id, req.body, actor ?? undefined);
          ...
          return reply
            .status(status)
            .send({ error: { code: status === 404 ? 'NOT_FOUND' : 'VALIDATION', message: msg } });
        }
      },
    );

    app.delete<{ Params: { id: string } }>('/config/services/:id', async (req, reply) => {
      const actor = (req.headers['x-actor'] as string) ?? null;
      ...
      async (req, reply) => {
        ...
        if (!svcId || !registry.has(svcId)) {
          return reply.status(409).send({
            error: { code: 'NOT_LIVE', message: 'service not live (disabled or not tracked)' },
          });
        }
        const svc = registry.get(svcId);
        const started = Date.now();
        const res = await svc.checkHealth(newSignal());
        const latencyMs = Date.now() - started;
        if (res.ok) return { data: { ok: true, latencyMs, snapshot: res.value } };
        return {
          data: {
            ok: false,
            latencyMs,
    ...
    app.post<{ Body: unknown }>('/config/import', async (req, reply) => {
      try {
        ...
        return reply.status(400).send({
          error: { code: 'VALIDATION', message: e instanceof Error ? e.message : 'invalid bundle' },
        });
      }
    });

    app.get<{ Querystring: { limit?: string } }>('/config/audit', async (req) => {
