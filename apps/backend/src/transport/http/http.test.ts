...
describe('http routes', () => {
  ...
  it('GET /services/:kind/health returns 404 for unknown kind', async () => {
    const app = await makeApp(new ServiceRegistry());
    const res = await app.inject({ method: 'GET', url: '/services/missing/health' });
  ...
  it('GET /services/:kind/stats ok', async () => {
    ...
    const app = await makeApp(r);
    const res = await app.inject({ method: 'GET', url: '/services/bitcoin/stats' });
    expect(res.statusCode).toBe(200);
    expect((res.json() as { data: { metrics: { count: number } } }).data.metrics.count).toBe(42);
    await app.close();
  });
  ...
  it('GET /services/:kind/health maps domain error to 503', async () => {
    ...
    const app = await makeApp(r);
    const res = await app.inject({ method: 'GET', url: '/services/bitcoin/health' });
  ...
  it('POST /services/:kind/control rejects missing action', async () => {
    ...
    r.register(new FakeSvc('bitcoin', 'main'));
    const app = await makeApp(r);
    const res = await app.inject({ method: 'POST', url: '/services/bitcoin/control', payload: {} });
  ...
  it('POST /services/:kind/control ok', async () => {
    ...
    r.register(new FakeSvc('bitcoin', 'main'));
    const app = await makeApp(r);
    const res = await app.inject({ method: 'POST', url: '/services/bitcoin/control', payload: { action: 'restart' } });
    expect(res.statusCode).toBe(200);
    await app.close();
  });
  ...
  it('GET /metrics returns snapshot', async () => {
    ...
    expect(res.statusCode).toBe(200);
    const body = res.json() as { breakers: unknown; process: { uptimeSec: number } };
