...
class AuditLogger {
  ...
  async initializeAuditDir() {
    try {
      ...
      logger.error("Failed to create audit directory", {
        error: error.message,
      });
    }
  }
  ...
  async log(event) {
    ...
    try {
      ...
      await appendFile(auditFile, JSON.stringify(logEntry) + "\n", "utf-8");
    } catch (error) {
...
export function auditMiddleware(req, res, next) {
  ...
  res.json = function (body) {
    ...
    if (req.user) {
      ...
      auditLogger
        .logDataAccess(req.user.username, req.ip, resource, action, {
