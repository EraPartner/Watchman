...
export const PerformantServiceCard = memo<PerformantServiceCardProps>(
  ({
    ...
    const {
      ...
      refetchInterval:
        priority === "high" ? 5000 : priority === "medium" ? 10000 : 15000,
      enabled: true,
    ...
    const statusMetrics = useMemo(() => {
      ...
      return {
        ...
        isHealthy:
          health.status === "online" && (health.responseTime || 0) < 1000,
        displayResponseTime: health.responseTime
    ...
    const formattedStats = useMemo(() => {
      ...
      return Object.entries(stats)
        ...
        .map(([key, value]) => ({
          ...
          isImportant: ["uptime", "connections", "queries", "blocks"].some(
            (important) => key.toLowerCase().includes(important),
          ),
        }));
    }, [stats, enableStats]);
          ...
          <div className="flex items-center gap-2">
            {statusMetrics?.performanceLevel && (
              <Badge
                variant="outline"
                className={`text-xs ${
                  statusMetrics.performanceLevel === "excellent"
                    ? "border-green-500 text-green-700"
                    : statusMetrics.performanceLevel === "good"
                      ? "border-blue-500 text-blue-700"
                      : statusMetrics.performanceLevel === "fair"
