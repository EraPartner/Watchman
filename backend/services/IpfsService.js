...
export default class IpfsService {
  constructor(config = {}) {
    this.apiUrl = config.apiUrl || process.env.IPFS_API_URL || 'http://127.0.0.1:5001';
    this.timeout = typeof config.timeout === 'number' ? config.timeout : (process.env.IPFS_TIMEOUT ? parseInt(process.env.IPFS_TIMEOUT) : 5000);
    this.forcePost = (config.forcePost !== undefined) ? config.forcePost : (process.env.IPFS_FORCE_POST === 'true');
    this.lastCheck = null;
  }
  ...
  async _fetch(path, method = undefined) {
    ...
    try {
      ...
      if (res && res.status === 405) {
        ...
        console.warn(`IPFS API ${path} returned 405 on ${firstMethod}, retrying with ${fallback}`);
  ...
  async checkHealth() {
    ...
    try {
      ...
      return {
        status: 'online',
        version: v && (v.Version || v.version) ? (v.Version || v.version) : 'unknown',
        lastCheck: checkedAt
      };
    } catch (error) {
      return {
  ...
  async getStats() {
    try {
      ...
      const [version, id, peersRes, repo, bwRes, bitswapRes] = await Promise.all([
      ...
      const repoStats = repo && (repo.RepoSize || repo.repoSize || repo.NumObjects || repo.numObjects) ? {
      ...
      const bw = bwRes && (bwRes.TotalIn !== undefined || bwRes.RateIn !== undefined) ? {
      ...
      return {
        version: version && (version.Version || version.version) ? (version.Version || version.version) : 'unknown',
        id: id && (id.ID || id.id) ? (id.ID || id.id) : null,
        addresses: id && id.Addresses ? id.Addresses : (id && id.addresses ? id.addresses : []),
        peers: peersCount,
        repo: repoStats,
        bw,
        bitswap
