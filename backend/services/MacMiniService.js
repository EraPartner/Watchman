...
class MacMiniService {
  constructor(options = {}) {
    ...
    this.sshPort = options.sshPort || parseInt(process.env.MACMINI_SSH_PORT || '22', 10);
    // Accept either MACMINI_SSH_KEY or MACMINI_SSH_KEY_PATH (existing .env uses MACMINI_SSH_KEY_PATH)
    this.sshKey = options.sshKey || process.env.MACMINI_SSH_KEY || process.env.MACMINI_SSH_KEY_PATH || null;
    // Accept password/passphrase from options or env. By default treat this as the private-key passphrase.
    this.sshPassword = options.sshPassword || process.env.MACMINI_SSH_PASSWORD || null;
    // If you explicitly want to use password authentication (not passphrase), set MACMINI_SSH_USE_PASSWORD=true
    this.sshUsePassword = (typeof options.sshUsePassword === 'boolean') ? options.sshUsePassword : (String(process.env.MACMINI_SSH_USE_PASSWORD || '').toLowerCase() === 'true');

    this.timeout = parseInt(options.timeout || process.env.MACMINI_TIMEOUT || '5000', 10);
  ...
  async pingHost() {
    ...
    for (const cmd of attempts) {
      try {
        const { stdout, stderr } = await execAsync(cmd, { timeout: this.timeout + 2000 });
        combinedStdout += `\n--- cmd: ${cmd} ---\n` + (stdout || '');
        combinedStderr += `\n--- cmd: ${cmd} ---\n` + (stderr || '');
    ...
    if (!combinedStdout && !combinedStderr) combinedStderr = 'No ping output captured; ping may be unavailable or blocked';
  ...
  buildSshCommand(cmd) {
    ...
    parts.push(`${this.sshUser}@${this.host}`, '"' + cmd.replace(/"/g, '\\"') + '"');
    return parts.join(' ');
  }
  ...
  runCmdOverSsh2(cmd) {
    return new Promise((resolve, reject) => {
      if (!this.useSSH) return reject(new Error('SSH not configured for Mac Mini'));
      ...
      if (this.sshKey) {
        try {
          ...
          if (!hasAgent) return reject(new Error('Failed to read private key for ssh2: ' + (err.message || err)));
        }
      }
    ...
    try {
      ...
      const result = {
        ...
        data: {
          host: this.host,
          ping: isOnline,
          pingOutput: (pingResult.stdout && pingResult.stdout.trim()) ? pingResult.stdout.trim() : (pingResult.stderr && pingResult.stderr.trim()) ? pingResult.stderr.trim() : null
        }
      };
  ...
  async runRemoteCommand(cmd) {
    ...
    const { stdout, stderr } = await execAsync(sshCmd, { timeout: this.timeout + 3000 });
  ...
  async getStats() {
    ...
    if (!this.useSSH) {
      return {
        message: 'SSH not configured; only health checks via ping are available'
      };
    }
    ...
    try {
      ...
      try {
        ...
        const tempCmd = 'export PATH="/opt/homebrew/bin:/usr/local/bin:$PATH"; which osx-cpu-temp && osx-cpu-temp';
      ...
      let cpuLoad = null;
      const loadMatch = uptimeOut && uptimeOut.match(/load averages?:?\s*([0-9.,\s]+)/i);
      if (loadMatch && loadMatch[1]) {
        const parts = loadMatch[1].split(/[ ,]+/).map(p => p.trim()).filter(Boolean);
      ...
      if (dfLines.length >= 2) {
        ...
        if (!Number.isNaN(totalK)) {
          ...
          const usagePercent = total > 0 ? Math.round((used / total) * 100) : null;
          disk = { total, used, free, usagePercent };
        }
      }

      const uptimeSeconds = parseUptimeSeconds(uptimeOut ? uptimeOut.trim() : null);
