...
class IPControlManager {
  ...
  async loadConfig() {
    try {
      ...
      logger.error('Failed to load IP control config', { error: error.message });
  ...
  async saveConfig() {
    try {
      ...
      logger.error('Failed to save IP control config', { error: error.message });
    }
  }
  ...
  tempBlock(ip, durationMs = 3600000) { // 1 hour default
  ...
  getStats() {
    return {
      whitelist: Array.from(this.whitelist),
      blacklist: Array.from(this.blacklist),
      tempBlacklist: Array.from(this.tempBlacklist.entries()).map(([ip, expiry]) => ({
        ip,
