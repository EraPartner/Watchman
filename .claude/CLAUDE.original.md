...
**Watchman** is a self-hosted service monitoring dashboard. It polls 14+ home-lab service types (Bitcoin node, IPFS, qBittorrent, Tor, AlbyHub, Synology, AdGuard, Homebridge, Roon, Philips Hue, Raspberry Pi, Mac Mini, Router, and more) and exposes a unified React dashboard with live status, metrics, and service controls.

The `docs/` directory is an Obsidian knowledge base — the authoritative source for architecture decisions, conventions, and project context.
...
2. **KB updater last** — After any code change, invoke the `watchman-kb-updater` agent before marking the task complete. This is mandatory, not optional.
...
- **ServiceManager** — Central orchestration point; all service calls route through it
...
- `async/await` for everything async
...
> Read `docs/INDEX.md`, the relevant ADRs in `docs/adr/`, and any related docs in `docs/architecture/`. Tell me what you understand about what I'm building and ask the three most important questions before we write any code.

**Significant architectural decision:**
> Write an ADR in `docs/adr/` following `docs/adr/template.md`. Document: what was decided, why, alternatives considered, and when to revisit.

**Hard bug:**
> Before reading code, check `docs/adr/` for decisions related to the affected system and `docs/troubleshooting.md` for known issues.
